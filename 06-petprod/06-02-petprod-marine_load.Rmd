## Refinery and Marine Loading Operations {#petprod-marine_load}

```{r petprod-marine_load-setup}
THESE_CATEGORIES <- c(
...
### Trends {#petprod-marine_load-trend}

#### History {-}

```{r petprod-marine_load-annual_emissions-ORG-chart, fig.height=6, out.height=600}
...
```{r petprod-marine_load-annual_emissions-PM-chart}
BY2011_annual_emission_data %>%
  filter_categories(
    CONSOLIDATED_CATEGORIES) %>%
  filter_pollutants(
...
```{r petprod-marine_load-growth-crosswalk}
...
```{r petprod-marine_load-growth-chart, fig.height=20, out.height=2000}
...
#### Control {- #petprod-marine_load-control}

```{r petprod-marine_load-lightering-control-chart}
...
Categories 86 - 87: Regulation 8-46 (Marine Vessel to Marine Vessel Loading) became effective on July 1, 1991. The overall control efficiency is estimated to be 95%.

```{r petprod-marine_load-ballast_vent-control-chart, fig.height=6, out.height=600}
BY2011_annual_control_factor_data %>%
  filter_categories(
...
Categories 88, 89, and 91: On December 7, 2005, District Regulation 8, Rule 44 (Marine Tank Vessel Operations) was amended to extend organic compound control requirements to ballasting (Categories 88 and 89), gas freeing, tank washing, and purging activities (Category 91) from venting tank emissions to the atmosphere. This amendment controls organic emissions from loading operations of gasoline, gasoline blended stock, aviation gas, JP4 jet fuel, crude oil and other liquids with a flash point below 100 oF. (Since diesel, residual, and distillate fuel oils have flash points above 100 ºF, these fuels from Category 90 were excluded from control requirements. The adoption of this amendment will control organic emissions by 95%.

```{r petprod-marine_load-barge_tanker-control-chart, fig.height=10, out.height=1000}
BY2011_annual_control_factor_data %>%
  filter_categories(
...
Categories 795 - 802: District Regulation 8-44 (Marine Vessel Loading Terminals) became effective July 1, 1991. The overall control efficiency is estimated to be 96%.
