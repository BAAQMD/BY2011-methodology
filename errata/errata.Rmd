...
- [General Refinery Emissions](#petref-fac-general-trend)
    - For category #10 Basic Processes, BY2011 NO~x~ emissions are much lower than DataBank numbers would indicate. SO~x~ and PM emissions, on the other hand, are consistent. CY1990-1995 PM~10~ emissions are low, compared to PY data.
    - For category #16 Other, CO~2~ emissions are large (~3 MMTCO2eq) and linearly increasing, which is mildly inconsistent with profiles for other pollutants in this category. Given the large quantity (~ 3 MMTCO2eq), this may merit a reassessment.
    - CY1990-2000 estimates of #15 Flares and Blowdown Systems may merit a reassessment.

- [All Other Chemical Plants](#chem-fac-other-trend): 10,000 ton/yr of CO emissions stops abruptly at CY1998. Few other point-source categories even approach this magnitude of CO emissions. This is traceable to a single facility. For perspective, all five refineries (all of their categories combined) emit about 2,000 to 4,000 ton/yr CO, and have since CY1990.

- [Portable Fuel Container Spillage](#petprod-portable-spillage-methodology): The text states that "CARB estimates 3.57 tons of uncontrolled ROG emissions per day statewide from portable fuel containers in 2011." That 3.57 ton/day is equal to `r as_units(3.57, "ton/day") %>% set_units("ton/yr") %>% drop_units() %>% formattable::accounting(digits = 0)` ton/yr. In the BY2011 inventory, the estimated CY2011 total for category #1434 Portable Fuel Container Spillage is `r BY2011_annual_emission_data %>% filter_categories(1434) %>% filter_years(CY(2011)) %>% filter_pollutants("ROG") %>% convert_emission_units(to = "ton/yr") %>% pull_total(ems_qty) %>% formattable::accounting(digits = 0)` ton/yr ROG. The Bay Area total should not be equal to the statewide total. (Perhaps the text is incorrect instead?)

- [Cogeneration](#SS-combust-cogen-trend): Estimated CY1990-2007 TOG emissions for category #292 Reciprocating Engines are too low, compared to CH~4~. (The CH~4~/TOG ratio for those years is well above 1, generally in the range of 3x to 30x.) About 1,000 ton/yr TOG and 100 ton/yr ROG may be missing for those years.

- The CO~2~/CO ratio for category [#293 Gas Fired Boilers](#SS-combust-power-boiler-trend) for CY1990-2000 appears to be far too low, given what is observed for subsequent decades. This could be resulting in the absence of 5-10 MMTCO2eq from the CY1990 BAAQMD inventory. An absence of CO~2~ of that magnitude, in CY1990, could be substantially distorting the answer to the key question of whether the District is "on track" to reach its CY2030 target.

- (Possibly related.) Emissions for [#294 Oil Fired Boilers](#SS-combust-power-boiler-trend) seem to be almost entirely absent from the BY2011 inventory. There are some very small emissions (~ 0.1 ton/yr of TOG and NO~x~) estimated for CY2008-2010, but that is it.

- [Oil Refineries - External Combustion](#SS-combust-petref)
    - Inventoried PM emissions from category #301 Solid Fuel increase suddenly from 3 ton/yr (until RY2008) to 300 ton/yr (RY2009 and later). This is entirely traceable to FCCU emissions at facility #10 Chevron Refinery.
    - Inventoried SO~2~ emissions from category #301 Solid Fuel decrease suddenly from 1500 ton/yr (until RY2008) to 150 ton/yr (RY2009 and later). Again, this is entirely traceable to FCCU emissions at facility #10 Chevron Refinery. Some explanation is probably merited.
    - Inventoried TOG emissions from category #299 Natural Gas & Other Gases increase by a factor of 2x beginning in RY2008. This is traceable to facilities #10 Chevron and #11 Shell, across a variety of `src_code`s, the largest being `C1130235`: CO-fueled boiler(s), 100-250 MMBTU/hr.
...
- [Other External Combustion - Natural Gas](#SS-combust-external-NG-trend): Throughput for categories #1590 and #1591 (NG Combustion, Commercial and Industrial) should be double-checked against CEC data, especially for years CY1990-2000. A large amount of GHG and (criteria-pollutant) emissions are at stake --- several MMTCO2eq and hundreds or thousands of tons of PM, TOG, and NO~x~, respectively. See the example charts for [Natural Gas Consumption](https://baaqmd.github.io/charting-annual-data/10-third_party.html#natural-gas-consumption) in the Charting Annual Data cookbook.

- [Ships](#mobile-marine-ship): All criteria-pollutant emissions are missing from "Ships in Transit, Container Ship, Main Engine, SFAB" (Category #2158) for years CY1990-2008. There are no emissions from either distillate fuel or residual fuel for those years.
...
- [Rubber Manufacturing](#indcom-mfg-rubber): Emissions of TOG and ROG classified as #46 Rubber Products Mfg (point) increase sharply at CY2011, from less than 1 ton/yr to 19 ton/yr.
...
- [POTWs](#waste-mgmt-POTW-growth): Projected growth for #50 POTWs (Point), at +17.8% (CY2030/2011), does not match the ABAG 2009 Total Population growth profile (+39.5%).
...
- [Industrial and Commercial Coatings: Marine](#petprod-coat-indcom-marine-trend): Categories #1930 and #1931 are missing emission data for CY2005-2008. These aren't zeros; there are just no rows in the `BY2011-annual-A.csv` file, for these categories, for CY2005-2008.
...
- [Solvent Cleaning](#petprod-clean-growth): Projected long-term growth for #1245 Other Handwiping (Point), at +9.7% (CY2030/2011) is inconsistent with the text ("ABAG 2009 Manufacturing Employment") and with #1935 Other Handwiping (Area), both of which are +28.4%.

- [Industrial/Commercial Coating](#petprod-coat-indcom): Projected long-term growth for [Auto Refinishing](#petprod-coat-indcom-auto-refinish) categories, at +17.8% (CY2030/2011), is inconsistent with projected long-term  growth for all other Industrial/Commercial Coating categories (+28.4%).
...
- [Power Plants: Gas/Oil Turbines](#SS-combust-power-turbine)
    - For category #297 Oil Fired Turbines, the CY1990-2010 decline in SO~2~ emissions does not appear to be explained by either the growth profile or the control profile (for this category, there are no control factors for SO~2~ in DataBank, only NO~x~). 
    - For category #1595 Gas Fired Turbines, a sharp rise and fall in CO, CY2002-2009, does not match other pollutants. 
    - For category #1597 Power Imports, [county fractions](#SS-combust-power-turbine-county-fractions) for CC and SOL are zero.
...
- [Recreational Boats](#mobile-marine-boat-recreation-trend): Emissions of many pollutants drop to exactly zero at five-year intervals (CY2020, 2025, 2030, ..., etc.), and then return to the previous trend. 

- [Lawn & Garden Equipment](#mobile-offroad-lawn-garden-trend): N~2~O emissions for a five-year interval (CY2026-2030) are exactly zero.

- [Transportation Refrigeration Units (TRUs)](#mobile-offroad-TRU-trend): Like [Lawn & Garden Equipment](#mobile-offroad-lawn-garden), N~2~O emissions for a five-year interval (CY2026-2030) are exactly zero.

- [Agricultural Equipment](#mobile-offroad-ag-trend): Like [TRUs](#mobile-offroad-TRU), N~2~O emissions for a five-year interval (CY2026-2030) are exactly zero. Also, #1646 Ag Equip (Diesel) is linked to growth profile #464 Commercial Boats. (?)
...
- [Animal Waste, Other](#misc-animal-other): County fractions (Table \@ref(tab:misc-animal-other-county-fraction-table) are inconsistent with the text.
...
- [Back-up Diesel Generator](#SS-combust-engine-crisis): Category #1598 should be (re)assessed.

#### Mobile Combustion {-}

- [Ships](#mobile-marine-ship): BY2011 categories are actually #2041:2220, not #1787:1887.
