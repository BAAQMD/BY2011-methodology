...
- Emission factors for #1936 Composting (Area) are time-varying (see [chart](#waste-mgmt-compost-emfac-chart)) but only seem to change during a three year period, CY2010-2012. Why?
...
- County fractions in Table \@ref(tab:misc-sewer-county-fraction-table) are inconsistent with county fractions computed directly from growth profile #657 Household Population. Compare: `DB_raw_growth_profiles(nested = FALSE) %>% filter(gpf_id == 657) %>% filter_years(CY(2011)) %>% transmute(gpf_id, gpf_name, cnty_abbr, cnty_frac = format_percentage(gf_qty / total(gf_qty), digits = 1)) %>% spread(cnty_abbr, cnty_frac)`. Perhaps this is due to the use of a subpopulation or another data source?
